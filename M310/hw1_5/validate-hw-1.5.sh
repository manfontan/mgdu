...
function mongoEval {
  local port=$1
  local script=$2
  echo `mongo --quiet --host database.m310.mongodb.university \
  --ssl --sslPEMKeyFile ~/shared/certs/client.pem \
  --sslCAFile ~/shared/certs/ca.pem --port $port --eval "$script"`
}
...
echo "{ unauthorizedStatus: $(getUnauthorizedStatus $primaryPort), \
memberStatuses: $(getMemberStatuses $primaryPort), \
users: $(getUsers $primaryPort) }"
